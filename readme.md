...
Client-side dynamic HTML imports in the frontend (essentially glorified iframes) may appear to be a questionable idea, but that really depends on the context. I originally came up with this custom elements as a way to modularize my collection of HTML-based slides about various overlapping topics, for which the element worked very well. I organized almost all of my slides into around 250 "stories" that I can now assemble into new presentations in minutes.
...
You can construct instances of the element by using the `HTMLHTMLImportElement` constructor:
...
* `importstart`: Fires when the element starts to load content, e.g. after a change to the attributes `src` or `selector` or after `load()` was called.
...
`HTMLHTMLImportElement` implements a DOM method called `reload()` which you can use to cause a re-load without having to change any attributes or properties. The method takes no arguments and returns a promise that resolves to an array of imported elements and the imported document's titles (just like the promise returned by `done` as described below).
...
The promises returned by `done` resolve to an array of data about what has been imported:
...
Any change to the attributes `src` or `selector` causes the imported content to update. The old nodes get removed and replaced by the newly imported content. This means that it's quite easy to "ajaxify" any old collection of static HTML documents:
...
`<html-import>` is available under two different licenses depending on how you plan to use it:
